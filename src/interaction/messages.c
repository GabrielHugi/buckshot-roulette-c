...
void printPlayOption() {
  char string[2][stdMaxChars];
  for (int i = 0; i < 2; i++) memset(string[i], 0, stdMaxChars);
  sprintf(string[0], "1 - shoot yourself");
  sprintf(string[1], "2 - shoot dealer");
  putInsideBox(string, 2, 1, 1);
  for (int i = 0; i < 2; i++) {
    printf("%s\n", string[i]);
  }
}
